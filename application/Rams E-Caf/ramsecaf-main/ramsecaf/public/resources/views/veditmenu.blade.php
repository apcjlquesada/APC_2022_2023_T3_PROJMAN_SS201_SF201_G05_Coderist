...
<body>
  ...
  <tbody>
    ...
    <form method="post" enctype="multipart/form-data" action="{{url('updatemenu')}}">
      ...
      <img src="{{url('images/'.$product->image)}}" class="rounded-circle mb-2" style="width: 100px; height: 100px; position: relative;" alt="Avatar" /></td>
      ...
      <td>
        <input type="text" value="{{$product->stocks}}" name="stocks" class="form-control">
      </td>
      <td>
        @if($product->isactive == 1)
        <a href="/updatestatus/{{$product->id}}" type="button" class="btn btn-danger" style="color:white">Deactivate</a>
        @else
        <a href="/updatestatus/{{$product->id}}" type="button" class="btn btn-warning">Activate</a>
        @endif
    </td>
